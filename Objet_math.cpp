...
VECDBL operator*(double d, VECDBL &vec) {
    ...
    pv = vec.v;
    ps = s.v;
    for (int i = 0; i < vec.n; i++) { *ps++ = *pv++ * d; }
    return (s);
}

VECDBL operator*(VECDBL &vec, double d) {
    VECDBL s(vec.n);
    double *pv, *ps;
    pv = vec.v;
    ps = s.v;
...
VECDBL operator*(MATDBL &m, VECDBL &v) {
    ...
    if (v.n == m.m) {
        ...
        cout << "*** Erreur : produit matrice par vecteur de tailles incompatibles";
        exit(0);
    }
    return (s);
}
...
void VECDBL::operator+=(VECDBL &vv) {
    ...
    pvv = vv.v;;
    if (vv.n == n)for (int i = 0; i < n; i++) { *pv++ += *pvv++; }
    else { cout << "*** Erreur : operateur += entre deux vecteurs de tailles differentes\n"; }
}
...
MATINT::MATINT(int a, int b) {
    ...
    for (int i = 0; i < n; i++) {
        mat[i] = new int[m];
        if (!mat[i]) {
            cout << "*** Erreur : plus d'esapace memoire" << "\n";
            exit(0);
        }
    }
